		...
		internal virtual T UnicastMessage<T>(IModbusMessage message) where T : IModbusMessage, new()
		{
			...
			do
			{
					...
					lock (_syncLock)
					{
						...
						do
						{
							...
							if (exceptionResponse != null)
							{
								...
								{
									Debug.WriteLine("Received ACKNOWLEDGE slave exception response, waiting {0} milliseconds and retrying to read response.", _waitToRetryMilliseconds);
				...
				catch (SlaveException se)
				{
					...
					Debug.WriteLine("Received SLAVE_DEVICE_BUSY exception response, waiting {0} milliseconds and resubmitting request.", _waitToRetryMilliseconds);
					Thread.Sleep(WaitToRetryMilliseconds);
				}
				catch (Exception e)
				{
